...
app.get("/", function (req, res) {
  ...
  const sql1 =
    "SELECT COUNT(*) TOT_CNT,        IFNULL(SUM(CASE WHEN A.COMPLETE_YN = 'Y' THEN 1 ELSE 0 END), 0) COMPLETE_CNT,        IFNULL(SUM(CASE WHEN A.COMPLETE_YN = 'N' THEN 1 ELSE 0 END), 0) NON_COMPLETE_CNT,        IFNULL(SUM(CASE WHEN A.ALERT_TYPE = '01' AND A.COMPLETE_YN != 'Y' THEN 1 ELSE 0 END), 0) HEART_CNT /* 심박동이상 */ ,        IFNULL(SUM(CASE WHEN A.ALERT_TYPE = '02' AND A.COMPLETE_YN != 'Y' THEN 1 ELSE 0 END), 0) SOS_CNT /* SOS */ ,        IFNULL(SUM(CASE WHEN A.ALERT_TYPE != '01' AND A.ALERT_TYPE != '02' AND A.COMPLETE_YN != 'Y' THEN 1 ELSE 0 END),0) HAZARD_CNT /* 경고알림 */   FROM TB_ALERT A  WHERE 1 = 1    AND A.ALERT_TYPE IN ('01', '02', '05', '06', '07', '08', '09', '10')    AND A.BIZ_ID = ?    AND A.COMPLETE_YN = 'N' /* 미조치된 건 */    AND A.REG_DT >= CONCAT(DATE_FORMAT(NOW(), '%Y-%m-%d'), ' 00:00:00');";
  const sql_for_total_list =
    "SELECT A.BIZ_ID,        A.ALERT_TYPE,        A.ALERT_SEQ,        A.DEVICE_ID,        A.TAG_ID,        IF(B.DEVICE_ID != '0', B.DEVICE_NUM, G.TAG_NUM) DEVICE_NUM/* 디바이스 번호 */,        C.USER_NM /* 사용자명 */,    C.USER_ID,        C.MOBILE,        F.AREA_NM /* 현재위치 */,        A.COMPLETE_YN,        A.REMARK,        CASE WHEN C.BIRTH IS NOT NULL AND C.BIRTH != '' THEN CONCAT(( DATE_FORMAT(NOW(), '%Y') - DATE_FORMAT(C.BIRTH, '%Y') ) + 1, '세')             ELSE ''         END AS AGE /* 나이 */,        C.GENDER,        CASE WHEN C.GENDER = 'M' THEN '남' ELSE '여' END GENDER_NM,        IF(C.DISEASE_YN = 'Y', 'Y', 'N')                DISEASE_YN/* 건강취약여부 */,        IF(C.RISK_WORKER_YN = 'Y', 'Y', 'N')            RISK_WORKER_YN/* 위험작업자여부 */,        DATE_FORMAT(A.REG_DT, '%Y-%m-%d %H:%i:%s')      REG_DT/* 등록일 : 위험 발생 시간 */,        F.AREA_NM /* 발생위치 */, A.LAT, A.LNG,        IF(F.HAZARD_YN = 'Y', '위험 구역', '')      '위험구역여부',        CASE WHEN A.LAT IS NOT NULL AND A.LNG IS NOT NULL THEN 'Y'             ELSE 'N'         END '지도보기가능여부'   FROM TB_ALERT A LEFT OUTER JOIN TB_DEVICE B ON A.DEVICE_ID = B.DEVICE_ID AND B.USE_YN = 'Y' 		  LEFT OUTER JOIN TB_USER C ON A.USER_ID = C.USER_ID AND C.USE_YN = 'Y' 		  LEFT OUTER JOIN TB_ENTRY_INOUT E ON E.ENTRY_TYPE = 'A' AND A.DEVICE_ID = E.DEVICE_ID AND A.USER_ID = E.USER_ID AND E.EFF_END_DT = '99991231235959' 		  LEFT OUTER JOIN TB_AREA F ON E.AREA_ID = F.AREA_ID AND F.USE_YN = 'Y' 		  LEFT OUTER JOIN TB_TAG G ON A.TAG_ID = G.TAG_ID AND G.USE_YN = 'Y'  WHERE 1 = 1    AND A.COMPLETE_YN = 'N' /* 미조치된 건 */    AND A.REG_DT > DATE_FORMAT(NOW(), '%Y-%m-%d 00:00:00')    AND A.ALERT_TYPE IN ( '01', '02', '05', '06', '07', '08', '09', '10' )    AND A.BIZ_ID = ?  ORDER  BY A.COMPLETE_YN;";

  try {
    jwt.verify(body.token, key.ACCESS_TOKEN_SECRET);
    db.query(sql1, BIZ_ID, (err, result) => {
      if (!err) {
        db.query(sql_for_total_list, BIZ_ID, (err2, result2) => {
          //console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
          //console.log(result2);
          if (!err2) {
            res.send({
              status: "WorkerWarning success",
              data: result,
              total_data: result2,
            });
          } else {
            console.log("WorkerWarning Data error2 : " + err2);

            res.send({ message: "fail", status: "fail" });
          }
        });
      } else {
        console.log("WorkerWarning Data error1");
        res.send({ message: "fail", status: "fail" });
      }
    });
  } catch (err) {
