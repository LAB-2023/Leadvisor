...
app.get("/", function (req, res) {
  ...
  const DetailedInfoSql =
    ...
    "FROM TB_USER A " +
    "LEFT OUTER JOIN TB_BIZ C ON A.BIZ_ID = C.BIZ_ID " +
    "LEFT OUTER JOIN TB_USER_STATUS D ON D.BIZ_ID = C.UP_BIZ_ID AND A.USER_STATUS = D.USER_STATUS AND D.USE_YN = 'Y' " +
    "LEFT OUTER JOIN TB_DEVICE E ON A.USER_ID = E.USER_ID AND E.USE_YN = 'Y' " +
  ...
  try {
    ...
    db.query(sql1, params, (err, result) => {
      if (!err) {
        db.query(DetailedInfoSql, params, (err2, result2) => {
          if (!err2) {
            console.log(result);
            res.send({
              status: "DailyWorkerStatus success",
              data: result,
              WorkerListData: result2,
            });
          } else {
            console.log("DailyWorkerStatus Data error");
