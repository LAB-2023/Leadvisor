  ...
  try {
    ...
    db.query(sql1, params, (err, result) => {
      if (!err) {
        console.log(result);
        res.send({
          status: "user schedule detail Data success",
          data: result,
        });
      } else {
        console.log("user schedule detail Data error");
